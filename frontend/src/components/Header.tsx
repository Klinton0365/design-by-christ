    ...
    <header className="absolute inset-x-0 top-0 z-30 w-full">
      <div className="mx-auto flex max-w-[1200px] items-center justify-between px-6 py-11">
        ...
        <nav className="flex items-center gap-8">
          <ul className="flex items-center gap-8 font-body text-[20px] text-dark">
            {navItems.map((item) => (
              <li key={item.label} className="group relative">
                ...
                {item.children && (
                  <ul className="invisible absolute left-1/2 top-full z-40 min-w-[200px] -translate-x-1/2 rounded-xl bg-white py-2 opacity-0 shadow-lg transition-all group-hover:visible group-hover:opacity-100">
          ...
          <button aria-label="Search" className="opacity-80 hover:opacity-100">
            <svg viewBox="0 0 21 21" className="h-5 w-5" fill="none">
              <circle cx="9" cy="9" r="7.25" stroke="#141414" strokeWidth="1.5" />
              <path d="M14.5 14.5 19 19" stroke="#141414" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </button>
        </nav>
      </div>
